...
public class GruppiGUI extends JFrame {
	...
	public GruppiGUI(String NU, String NG) {
        ...
        NomeGruppo.setBounds(309, 11, 202, 38);
        ...
        for(int i=0; i<Res_Contenuti_Gruppi.size(); i++) {
        	 textArea.append(Res_Contenuti_Gruppi.get(i).getPubblicatore()+": "+Res_Contenuti_Gruppi.get(i).getTesto()+"\n");
        }
        ...
        Rimuovi_Post.addActionListener(new ActionListener() {
        	public void actionPerformed(ActionEvent e) {
        		...
        		int DelPost = Integer.parseInt(JOptionPane.showInputDialog(Rimuovi_Post, "Quale post vuoi eliminare?", "Elimina un post", JOptionPane.QUESTION_MESSAGE)); 
        		C.DelContenuto(DelPost);
        		textArea.setText(""); //elimina tutti i post
        		
        		//riprendo i post aggiornati (soluzione da rivedere)
        		Res_Contenuti_Gruppi = C.SelAllContenutiGruppo(NG); 
                for(int i=0; i<Res_Contenuti_Gruppi.size(); i++) {
                	 textArea.append(Res_Contenuti_Gruppi.get(i).getPubblicatore()+": "+Res_Contenuti_Gruppi.get(i).getTesto()+"\n");
                }
